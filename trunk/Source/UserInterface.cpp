...
CEGUI::Window* UI::getWindowFromName(const char* name)
{
	if (!g_pWm)
		return NULL;
...
CEGUI::Window* UI::createWindow(const char* name)
{
	if (!g_pWm)
		return NULL;

	g_pWm->createWindow(CEGUI::String("DefaultWindow"), CEGUI::String(name));
}
...
void UI::updateLatency(const double latency)
{
	if (g_pWm->isWindowPresent(CEGUI::String("666_Multiplayer_InGame_Latency"))) {
		...
		char buf[80];
		sprintf(buf, "%3i ms", (int)(latency * 1000));
		pLatency->setText(CEGUI::String("Latency: ") + buf);
	}
}

void UI::showChatWindow(bool visible)
{
...
bool UI::mouseEventChat(const CEGUI::EventArgs& args)
{
...
bool UI::keyboardEventChat(const CEGUI::EventArgs& args)
{
...
bool UI::mouseEventMultiplayer(const CEGUI::EventArgs& args)
{
...
bool UI::mouseEventCancel(const CEGUI::EventArgs& args)
{
	CEGUI::Window* settings = getWindowFromName("666_Multiplayer_ConnectionInfo");
	if (settings) {
...
bool UI::mouseEventConnect(const CEGUI::EventArgs& args)
{
	CEGUI::Window* settings = getWindowFromName("666_Multiplayer_ConnectionInfo");
	if (settings) {
		settings->setVisible(false);
...
bool UI::mouseEventHost(const CEGUI::EventArgs& args)
{
	CEGUI::Window* settings = getWindowFromName("666_Multiplayer_ConnectionInfo");
	if (settings) {
		settings->setVisible(false);
...
void UI::dumpToLog(CEGUI::Window* window, int level)
{
	CEGUI::Window* win;
	char buf[80] = {0};
...
void UI::dumpToFile(const char* filename)
{
